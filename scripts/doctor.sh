...
if [ "$PLATFORM" = "Darwin" ]; then
  ...
  if [ -f /etc/os-release ]; then
    . /etc/os-release
    check "Linux" "1" "${PRETTY_NAME:-$ID $VERSION_ID}"
  else
    check "Linux" "1" "$(uname -r)"
  fi
else
  check "Platform" "0" "$(uname) — Clui CC requires macOS or Linux"
fi
...
if command -v node &>/dev/null; then
  ...
  if version_gte "$node_ver" "18.0.0"; then
    ...
    if [ "$PLATFORM" = "Linux" ]; then
      check "Node.js" "0" "v$node_ver — requires 18+ — install via nvm or package manager"
    else
  ...
  if [ "$PLATFORM" = "Linux" ]; then
    check "Node.js" "0" "not found — install via nvm or package manager"
  else
...
if command -v npm &>/dev/null; then
  ...
  check "npm" "0" "not found — install Node.js"
fi
...
if command -v python3 &>/dev/null; then
  ...
  if [ "$PLATFORM" = "Linux" ]; then
    check "Python 3" "0" "not found — sudo apt install python3 (or equivalent)"
  else
...
if [ "$PLATFORM" = "Darwin" ]; then
  ...
  if command -v g++ &>/dev/null; then
    cver=$(g++ --version 2>&1 | head -1)
    check "C++ compiler" "1" "$cver"
  elif command -v clang++ &>/dev/null; then
    cver=$(clang++ --version 2>&1 | head -1)
    check "C++ compiler" "1" "$cver"
  else
    check "C++ compiler" "0" "not found — sudo apt install build-essential (or equivalent)"
  fi
  ...
  if command -v make &>/dev/null; then
    ...
    check "make" "0" "not found — sudo apt install build-essential (or equivalent)"
  fi

  # pkg-config (needed for node-pty native build)
  if command -v pkg-config &>/dev/null; then
    check "pkg-config" "1" "$(pkg-config --version)"
  else
    check "pkg-config" "0" "not found — sudo apt install pkg-config (or equivalent)"
  fi
fi
